...
class Field:
    ...
    def validate(self, value):
        ...
        elif self.type == "timeInvl":
            return isinstance(value, tuple) and len(value) == 2 and all(isinstance(v, time) for v in value)
        return False


class Record:
    def __init__(self, data):
        self.data = data  # Словник значень для кожного поля
...
class Table:
    def __init__(self, name):
        self.name = name
        self.fields = []  # Список полів (атрибутів)
        self.records = []  # Список записів у таблиці
    ...
    def add_record(self, record):
        ...
        if record.validate(self.fields):  # Перевірка на валідність запису
    ...
    def to_dict(self):
        return {
            ...
            "records": [record.data for record in self.records]  # Додаємо записи
        }
    ...
    def view_records(self):
        return [record.data for record in self.records] if hasattr(self, 'records') else []
...
class Database:
    ...
    def join_tables(self, table1_name, table2_name, common_field_name):
        ...
        # Отримання записів з обох таблиць
        ...
        for record1 in records1:
            for record2 in records2:
                # Перевірка значень у спільних полях
